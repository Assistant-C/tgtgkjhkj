...
function draw()
{
   ...
   for(let i=0; i<pipe.length; i++)
   {
      ...
      if( birdX + bird.width >= pipe[i].x && birdX <= pipe[i].x + pipeU.width
         &&(birdY <= pipe[i].y + pipeU.height || birdY + bird.height >= pipe[i].y + pipeU.height + gap) 
   ...
   dim.fillStyle = "#000"
   dim.font = "24px Arial"
   dim.fillText("Cчет: " + score, 10,canvas.height - 20)
